import { RepositorySnapshot, sha256Pure } from '@ayatlas/snapshot-model';
import fs from 'fs';
 ...
 */
export class LocalReadOnlyRepositorySource implements ReadOnlyRepositorySource {
  ...
  async resolveRef(ref = 'main'): Promise<ResolvedRepositoryRef> {
  ...
  async getTree(_ref = 'main'): Promise<{ entries: TreeEntry[]; isTruncated: boolean }> {
  ...
  async getFile(pathStr: string, _ref = 'main'): Promise<FileContent> {
