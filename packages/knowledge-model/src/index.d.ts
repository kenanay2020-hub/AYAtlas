 ...
 */
export type AssertionKind = 'REPOSITORY_FACT' | 'CANONICAL_STATUS' | 'HISTORICAL_RECORD' | 'DERIVED_RELATION' | 'EDUCATIONAL_EXPLANATION' | 'INFERENCE' | 'UNKNOWN';
/**
...
export interface MultiAxisStatus {
    ...
    evidence: 'NO_EVIDENCE' | 'OBSERVED_ARTIFACT' | 'LOCAL_VALIDATION_PASS' | 'REMOTE_WORKFLOW_PASS' | 'EXACT_SUBJECT_BOUND' | 'GOVERNANCE_REVIEWED' | 'ACCEPTANCE_PENDING' | 'ACCEPTED_EVIDENCE';
}
/**
 * Entity Types in Knowledge Graph
 */
export type EntityType = 'REPOSITORY' | 'SNAPSHOT' | 'DIRECTORY' | 'SOURCE_FILE' | 'COMPONENT' | 'ARCHITECTURE_LAYER' | 'PHASE' | 'DOCUMENT' | 'DECISION' | 'EVIDENCE_ARTIFACT' | 'WORKFLOW' | 'CI_GATE' | 'CONCEPT' | 'SYSCALL' | 'CRATE' | 'RUNTIME_SERVICE';
/**
 * Controlled Relation Vocabulary
 */
export type RelationType = 'CONTAINS' | 'DEPENDS_ON' | 'USES' | 'PRODUCES' | 'SUBMITS_TO' | 'VERIFIED_BY' | 'VALIDATED_BY' | 'GUARDED_BY' | 'GOVERNED_BY' | 'DOCUMENTED_BY' | 'INTRODUCED_IN' | 'CLOSED_IN' | 'SUPERSEDED_BY' | 'REFERENCES' | 'BOUND_TO' | 'DOES_NOT_AUTHORIZE';
/**
 * Knowledge Assertion
 */
 ...
 * Roadmap Schemas
 */
export type RoadmapKind = 'AYATLAS_PRODUCT' | 'AYKENOS_CANONICAL' | 'ANALYSIS_BACKLOG' | 'ARCHIVED';
export type RoadmapItemStatus = 'BACKLOG' | 'READY' | 'IN_PROGRESS' | 'BLOCKED' | 'REVIEW' | 'DONE' | 'CANCELLED';
