export const AYKENOS_TECHNICAL_CATALOG = [
...
export function resolveCatalogWithSnapshot(snapshot) {
    if (!snapshot)
        return AYKENOS_TECHNICAL_CATALOG;
    const filePaths = snapshot.files.map((f) => f.path);
    return AYKENOS_TECHNICAL_CATALOG.map((item) => {
        const matched = item.candidatePaths.filter((cp) => filePaths.some((fp) => fp.startsWith(cp) || fp.includes(cp)));
...
//# sourceMappingURL=aykenosCatalog.js.map